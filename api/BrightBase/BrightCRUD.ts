...
> {
  name: string
  private table: ReturnType<typeof supabase.from>
  ...
  private reCreateTable() {
    this.table = supabase.from(this.name)
  }
   ...
   * @example
   * userTable.create({ name: 'John Doe', email: 'john@example.com' }).then((users) => console.console.log(users)).catch(err => wetToast(err.message, { icon: '❌' })
   */
  async create<
  ...
  >(record: CreateTableRecord | CreateTableRecord[], schema?: ZodSchema<unknown>) {
    ...
    if (error) {
      console.log('Error creating record in table "%s": %s', this.name, error.message)
      throw new Error(error.message)
    }
    console.log('Record created in table "%s": ', this.name, record)
   ...
   * @example
   * userTable.read({ name: 'John Doe' }, { limit: 10 }).then((users) => console.console.log(users)).catch(err => wetToast(err.message, { icon: '❌' })
   */
  async read(
    ...
    if (error) {
      console.log('Error reading records in table "%s": %s', this.name, error.message)
      throw new Error(error.message)
    }

    if (!data) throw Error('No Data was found.')

    console.log('Read records from "%s" table with these filters and options: %o \n Data: %o', this.name, { filters, options }, data)
   ...
   * @example
   * userTable.update('id_of_record', { name: 'Jane Doe' }).then((users) => console.console.log(users)).catch(err => wetToast(err.message, { icon: '❌' })
   */
  async update(id: TableRecord['id'], updates: Partial<TableRecord>, schema?: ZodSchema<unknown>) {
    ...
    if (error) {
      console.log('Error updating record with id: "%s" in table "%s"', id, this.name, '\n', error.message)
      throw new Error(error.message)
    }

    console.log('Record(s) in "%s" table updated: %o', this.name, data)
    return data as TableRecord[]
  }
   ...
   * @example
   * userTable.delete('id_of_record').then((users) => console.console.log(users)).catch(err => wetToast(err.message, { icon: '❌' })
   */
  async delete(id: TableRecord['id']) {
    ...
    if (error) {
      console.log('Error deleting record: %s', id, '\n', error.message)
      throw new Error(error.message)
    }
